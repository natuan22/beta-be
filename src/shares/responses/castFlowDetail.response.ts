export class CastFlowDetailResponse {
  ...
  constructor(data?: CastFlowDetailResponse, type?: string, is_chart?: number) {
    switch (type) {
      case 'Ngân hàng':
        this.name =
          data?.name == 'Lưu chuyển tiền từ hoạt động kinh doanh' ||
      ...
      case 'Dịch vụ tài chính':
        this.name =
          ...
          data?.name == 'Tiền gửi ngân hàng cuối kỳ'
            ? data.name.toUpperCase()
            : data?.name;
        if (data.name == 'Cac khoan tuong duong tien dau ky')
          this.name = 'Các khoản tương đương tiền đầu kỳ';
        if (data.name == 'Cac khoan tuong duong tien cuoi ky')
          this.name = 'Các khoản tương đương tiền cuối kỳ';
        if (data.name == 'Anh huong dau ky')
          this.name =
            'Ảnh hưởng của thay đổi tỷ giá hối đoái quy đổi ngoại tệ đầu kỳ';
        if (data.name == 'Anh huong cuoi ky')
          this.name =
  ...
  static mapToList(
    data?: CastFlowDetailResponse[],
    is_chart?: number,
    type?: string,
  ) {
    return data.map(
      (item) =>
        new CastFlowDetailResponse(
          { ...item, name: is_chart ? item.name.toUpperCase() : item.name },
          type,
          is_chart,
        ),
    );
