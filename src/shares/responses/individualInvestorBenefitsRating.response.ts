...
export class IndividualInvestorBenefitsRatingResponse {
  static mapToList(
    ...
    co_tuc: number,
    ty_le_co_tuc: number,
    month_6_industry: number,
    year_1_industry: number,
    year_2_industry: number,
    year_4_industry: number,
    co_tuc_industry: number,
    ty_le_co_tuc_industry: number,
    month_6_all: number,
    year_1_all: number,
    year_2_all: number,
    year_4_all: number,
    co_tuc_all: number,
    ty_le_co_tuc_all: number,
  ) {
    const data = [
      {
        name: 'Tăng trưởng thị giá cổ phiếu',
        value: UtilCommonTemplate.checkStarCommonV2(
          month_6 + year_1 + year_2 + year_4,
          4,
        ),
        child: [
          {
    ...
    const totalStarIndustry = UtilCommonTemplate.checkStarCommon(
      UtilCommonTemplate.checkStarCommonV2(
        month_6_industry + year_1_industry + year_2_industry + year_4_industry,
        4,
      ) +
        UtilCommonTemplate.checkStarCommon(
          co_tuc_industry + ty_le_co_tuc_industry,
          2,
        ),
      2,
    );
    const totalStarAll = UtilCommonTemplate.checkStarCommon(
      UtilCommonTemplate.checkStarCommonV2(
        month_6_all + year_1_all + year_2_all + year_4_all,
        4,
      ) + UtilCommonTemplate.checkStarCommon(co_tuc_all + ty_le_co_tuc_all, 2),
      2,
    );
    return {
      totalStar,
