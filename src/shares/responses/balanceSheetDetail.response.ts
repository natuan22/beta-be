export class BalanceSheetDetailResponse {
  ...
  constructor(
    data?: BalanceSheetDetailResponse,
    is_chart?: number,
    type?: string,
  ) {
    this.name = data?.name || '';
    ...
    if (is_chart) {
      if (
        ...
        this.name =
          data.name.toLowerCase().charAt(0).toUpperCase() +
          data.name.slice(1).toLowerCase();
      if (data.name == 'no phai tra')
        this.name = 'Nợ phải trả trên tổng nguồn vốn (%)';
      if (data.name == 'von so huu')
        this.name = 'Vốn chủ sở hữu trên tổng nguồn vốn (%)';
      ...
      switch (type) {
        case 'Bảo hiểm':
          if (
            data.name == 'Tài sản lưu động và đầu tư ngắn hạn' ||
  ...
  static mapToList(
    ...
    return data.map(
      (item) => new BalanceSheetDetailResponse(item, is_chart, type),
    );
  }
}
