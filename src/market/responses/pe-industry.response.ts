...
import { IPEIndustry } from '../interfaces/pe-pb-industry-interface';

export class PEIndustryResponse {
  @ApiProperty({
    type: String,
    example: 'ACB',
  })
  ...
  public mapToList(data?: IPEIndustry[], type: number = 0) {
    return data?.map((item) => new PEIndustryResponse(item, type));
  }
}

export class PEIndustrySwagger {
  @ApiProperty({
    type: PEIndustryResponse,
    isArray: true,
  })
  data: PEIndustryResponse[];
}
