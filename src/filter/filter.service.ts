  ...
  async get() {
    try {
      ...
      const query = `
        ...
        max_dates AS (
          SELECT MAX(date) AS max_ngay FROM tradeIntraday.dbo.tickerTransVNDIntraday
        ),
        max_foreign_date AS (
          SELECT MAX(date) AS max_foreign_date FROM marketTrade.dbo.[foreign] WHERE type = 'STOCK'
        ),
        min_max AS (
        ...
        temp AS (
          SELECT f.code, f.floor, f.TyleNDTNNdangnamgiu, f.closePrice, f.min_1_week, f.max_1_week, f.min_1_month, f.max_1_month, f.min_3_month, f.max_3_month, 
                 ...
                 LEAD(f.ma10) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ma10_pre, LEAD(f.ma20) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ma20_pre,
                 LEAD(f.ma50) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ma50_pre, LEAD(f.ma100) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ma100_pre,
                 LEAD(f.ma200) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ma200_pre, LEAD(f.ema5) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ema5_pre,
                 LEAD(f.ema10) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ema10_pre, LEAD(f.ema20) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ema20_pre,
                 LEAD(f.ema50) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ema50_pre, LEAD(f.ema100) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ema100_pre,
                 LEAD(f.ema200) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS ema200_pre, LEAD(f.rsi) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS rsi_pre,
                 ...
                 LEAD(f.TinHieuChiBaoKyThuat) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS tech_pre, LEAD(f.TinHieuDuongXuHuong) OVER (PARTITION BY f.code ORDER BY f.date DESC) AS trend_pre,
      ...
      const query_2 = `
        ...
        average_volumes AS (
          SELECT code, AVG(CASE WHEN rn <= 5 THEN totalVol END) AS avg_totalVol_5d, AVG(CASE WHEN rn <= 10 THEN totalVol END) AS avg_totalVol_10d, 
                 AVG(CASE WHEN rn <= 20 THEN totalVol END) AS avg_totalVol_20d, AVG(CASE WHEN rn <= 60 THEN totalVol END) AS avg_totalVol_60d
          FROM ranked_trades
          GROUP BY code
        )
      ...
      const query_3 = `
        WITH yearQuarter4 AS (
          SELECT AVG(grossProfitMargin) AS grossProfitMarginQuarter, AVG(netProfitMargin) AS netProfitMarginQuarter, AVG(EBITDAMargin) AS EBITDAMarginQuarter, code
          FROM financialReport.dbo.calBCTC
          WHERE yearQuarter IN (SELECT DISTINCT TOP 4 yearQuarter FROM financialReport.dbo.calBCTC ORDER BY yearQuarter DESC)
          GROUP BY code
        )
        SELECT c.code, c.grossProfitMargin AS grossProfitMarginYear, y.grossProfitMarginQuarter, c.netProfitMargin AS netProfitMarginYear, y.netProfitMarginQuarter, c.EBITDAMargin, 
               y.EBITDAMarginQuarter, c.currentRatio, c.quickRatio, c.interestCoverageRatio, c.DE, c.totalDebtToTotalAssets, c.ATR, c.VongQuayTaiSanNganHan, c.VongQuayCacKhoanPhaiThu, 
               c.VongQuayHangTonKho, c.NoTraLaiDivVonChuSoHuu, c.NoTraLaiDivTongTaiSan
      ...
      const query_4 = `
        ...
        sum_roaa AS (
          SELECT code, SUM(roaa) AS roaa, SUM(roae) AS roae FROM filtered_roae WHERE rn <= 4 GROUP BY code
        )
      ...
      const query_5 = `
        WITH ranked_trades AS (
          SELECT code, omVol, date, ROW_NUMBER() OVER (PARTITION BY code ORDER BY date DESC) AS rn
          FROM marketTrade.dbo.tickerTradeVND
          WHERE type = 'STOCK'
        ),
        average_volumes AS (
          SELECT code, AVG(CASE WHEN rn > 1 AND rn <= 6 THEN omVol END) AS avg_totalVol_5d_excluding_last, AVG(CASE WHEN rn > 1 AND rn <= 11 THEN omVol END) AS avg_totalVol_10d_excluding_last,
                  AVG(CASE WHEN rn > 1 AND rn <= 21 THEN omVol END) AS avg_totalVol_20d_excluding_last, AVG(CASE WHEN rn > 1 AND rn <= 61 THEN omVol END) AS avg_totalVol_60d_excluding_last,
                  MAX(CASE WHEN rn = 1 THEN omVol END) AS last_totalVol
          FROM ranked_trades
          GROUP BY code
        ),
        percent_changes AS (
          SELECT code, avg_totalVol_5d_excluding_last, avg_totalVol_10d_excluding_last, avg_totalVol_20d_excluding_last, avg_totalVol_60d_excluding_last, last_totalVol,
        ...
        SELECT code, pct_change_5d AS perChangeOmVol_5d, pct_change_10d AS perChangeOmVol_10d, pct_change_20d AS perChangeOmVol_20d, pct_change_60d AS perChangeOmVol_60d
      ...
      const query_6 = `
        WITH ranked_eps AS (
          SELECT code, EPS, yearQuarter, LEAD(EPS) OVER (PARTITION BY code ORDER BY yearQuarter DESC) AS next_EPS
          FROM RATIO.dbo.ratioInYearQuarter 
          WHERE RIGHT(yearQuarter, 1) <> '0' AND type = 'STOCK'
        ),
        temp AS (
          SELECT code, yearQuarter, (EPS - next_EPS) / NULLIF(next_EPS, 0) * 100 AS perEPS -- Sử dụng NULLIF để tránh chia cho 0
          FROM ranked_eps
        )
