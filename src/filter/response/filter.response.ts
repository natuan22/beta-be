...
export class FilterResponse {
  ...
  changeStatusTech: number;
  changeStatusTrend: number;
  changeStatusGeneral: number;
  ...
  private calculateChartValues = (data) => {
    ...
    for (let i = 0; i < resultArray.length - 2; i++) {
      if (value >= resultArray[i] && value <= resultArray[i + 1]) {
        arr = [i, i + 1];
        break;
      }
    }
  ...
  private calChangeStatus(now: number, pre: number) {
    ...
    switch (value) {
      ...
      case '1,2':
        return 1; // Tiêu cực sang trung lập
      case '2,3':
        return 2; // Trung lập sang Tích cực
      case '3,4':
        return 3; // Tích cực sang rất tích cực
      case '4,3':
        return 4; // Rất tích cực sang tích cực
      case '3,2':
        return 5; // Tích cực sang trung lập
      case '2,1':
        return 6; // Trung lập sang tiêu cực
      case '1,0':
        return 7; // Tiêu cực về rất tiêu cực
      default:
        return 8; //Không chuyển trạng thái
    }
  }
}
