...
export class MarketVolatilityKafkaResponse {
  ...
  constructor(data?: DomesticIndexKafkaInterface) {
    ...
    this.day_change_percent = data?.['%1D']
      ? +data?.['%1D'].slice(0, data?.['%1D'].length - 1)
      : 0;
    this.week_change_percent = data?.['%5D']
      ? +data?.['%5D'].slice(0, data?.['%5D'].length - 1)
      : 0;
    this.month_change_percent = data?.['%MTD']
      ? +data?.['%MTD'].slice(0, data?.['%MTD'].length - 1)
      : 0;
    this.year_change_percent = data?.['%YTD']
      ? +data?.['%YTD'].slice(0, data?.['%YTD'].length - 1)
      : 0;
  }
