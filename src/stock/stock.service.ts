...
export class StockService {
  ...
  async getMaterialPriceV2() {
    try {
      const query = `
        WITH mx_date_h AS (
          SELECT MAX(lastUpdated) AS date, name AS code
          FROM macroEconomic.dbo.HangHoa
          WHERE unit != '' AND id IS NOT NULL 
          GROUP BY name
        ),
        temp AS (
          SELECT name, closePrice, date,
        ...
        ytd_date AS (
          SELECT MIN(date) AS date, name
          FROM macroEconomic.dbo.WorldIndices
          WHERE YEAR(date) = YEAR(GETDATE())
          GROUP BY name
        ),
        ytd_price AS (
          SELECT t.name, t.date, t.closePrice
          FROM temp t
          INNER JOIN ytd_date y ON t.name = y.name AND t.date = y.date
        ),
        temp_1 AS (
          SELECT code, closePrice, date,
        ...
        ytd_date_1 AS (
          SELECT MIN(date) AS date, code
          FROM marketTrade.dbo.indexTradeVND
          WHERE YEAR(date) = YEAR(GETDATE())
          GROUP BY code
        ),
        ytd_price_1 AS (
          SELECT t.code, t.date, t.closePrice
          FROM temp_1 t
          INNER JOIN ytd_date_1 y ON t.code = y.code AND t.date = y.date
        )
        SELECT t.code AS ticker, t.date AS date_time, t.closePrice AS diemso, t.prev AS percent_d, t.week AS percent_w, t.month AS percent_m, (t.closePrice - y.closePrice) / y.closePrice * 100 AS percent_ytd
        FROM temp_1 t
        INNER JOIN max_date_1 m ON t.date = m.date AND t.code = m.code
        INNER JOIN ytd_price_1 y ON t.code = y.code

        UNION ALL

        SELECT t.name AS ticker, t.date AS date_time, t.closePrice AS diemso, t.prev AS percent_d, t.week AS percent_w, t.month AS percent_m, (t.closePrice - y.closePrice) / y.closePrice * 100 AS percent_ytd
        FROM temp t
        INNER JOIN max_date m ON t.date = m.date AND t.name = m.name
        INNER JOIN ytd_price y ON t.name = y.name

        UNION ALL

        SELECT name AS ticker, lastUpdated AS date_time, price AS diemso,
        ...
        INNER JOIN mx_date_h m ON h.lastUpdated = m.date AND h.name = m.code
      `;
