...
export class MerchandisePriceResponse {
  ...
  constructor(data: MerchandisePriceInterface, type: number) {
    ...
    this.Day = data?.Day
      ? +type
        ? Number(data?.Day).toFixed(2) + '%'
        : data?.Day.split(' (')[1].slice(0, data?.Day.split(' (')[1].length - 1)
      : '0%';
    this.MTD = data?.MTD
      ? data?.MTD.split(' (')[1].slice(0, data?.MTD.split(' (')[1].length - 1)
      : '';
    this.YTD = data?.YTD
      ? data?.YTD.split(' (')[1].slice(0, data?.YTD.split(' (')[1].length - 1)
      : '';
  }
