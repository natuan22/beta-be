...
export class InternationalSubResponse {
  ...
  constructor(data?: any) {
    ...
    this.percent_d = data?.percent_d
      ? +data?.percent_d
          .split(' (')[1]
          .slice(0, data?.percent_d.split(' (')[1].length - 2)
      : 0;
    this.percent_w = data?.percent_w
      ? +data?.percent_w
          .split(' (')[1]
          .slice(0, data?.percent_w.split(' (')[1].length - 2)
      : 0;
    this.percent_m = data?.percent_m
      ? +data?.percent_m
          .split(' (')[1]
          .slice(0, data?.percent_m.split(' (')[1].length - 2)
      : 0;
    this.percent_ytd = data?.percent_ytd
      ? +data?.percent_ytd
          .split(' (')[1]
          .slice(0, data?.percent_ytd.split(' (')[1].length - 2)
      : 0;
  }
