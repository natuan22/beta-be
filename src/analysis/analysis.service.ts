...
export class AnalysisService {
  ...
  async getAnalysis(q: AnalysisDto) {
    const redisData = await this.redis.get(
      `${RedisKeys.analysisReport}:${q.type}`,
    );
    ...
    const getMainTitle = (title: any) => {
      ...
      if (title.includes('Bản tin thị trường cuối ngày'))
        return 'Bản tin thị trường cuối ngày';
      if (title.includes('Diễn biến thị trường phiên sáng'))
        return 'Diễn biến thị trường phiên sáng';
      if (title.includes('Bản tin trước giờ giao dịch'))
        return 'Bản tin trước giờ giao dịch';
      return title;
    };
    ...
    await this.redis.set(`${RedisKeys.analysisReport}:${q.type}`, dataMapped, {
